...
3. **CI 三条腿都不起 GUI，也不碰真实回收站与第二物理卷**：现读 `.github/workflows/ci.yml`，三腿的 step 只有 `go vet` / `go test -race` / `npm` 构建 / `fdd-cli` 冒烟（跨卷冒烟刻意只在 ubuntu 跑），同一文件头注"尚未纳入"里自认两条——"macOS/Linux 真实回收站与 GUI 端到端：需人工或桌面会话""macOS 侧跨卷冒烟：harness 依赖 losetup/mount -o loop，仅 Linux 可用"。⇒ **窗口前置/合并、真实回收站、真两卷的 EXDEV 三类格在三腿上都是零读数**，darwin 与 linux 在这一点上跟 windows 处境相同；这就是本文从单腿扩成三腿的全部理由（04 §6.36 五的 ★ 格、P10 末尾的 ★ 注）。
...
- **卷**：至少两个**不同文件系统**的挂载点（第二块盘或 `losetup -f --show` + `mkfs` 的 loop 卷即可），LNX-2/LNX-4 靠它。
