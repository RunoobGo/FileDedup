...
### 26.7 补 M84 的改动面被高估了（同批追记，取证后再开一轮码）

- ★ **遍历从不跟随符号链接**：`internal/scanner/scanner.go:328-330` 对 `ModeSymlink` 条目直接
  `continue`，且全包 `grep EvalSymlinks` **零命中** ⇒ 一次 `q.push` 只发生在根入队处（`:260 submit`）。
  推论：路径里可能带"用户给的链接名"的**只有根那一段**，其下所有 `full` 都是从根纯拼接下来的。
  ⇒ 登记设想的"队列元素由 `string` 变结构体、`visited`/`rootsUnder`/剪枝全部分叉"**并不需要**，
  需要第二键的位置只有一个（根侧清单判据 `:240`）。
- ★ 真正的阻塞点是清单条目本身的意图，不是实现难度：`sysguard.go:145`
  `{kind: eAbsPath, plat: pDarwin, name: "/private", why: "var/tmp/folders 等运行时目录的真身"}`
  —— 这条**故意**把用户临时目录的真身划进保护范围。于是"按真实路径判清单"一旦开启，
  darwin 上每一个 `t.TempDir()` 根（`/var/folders/...` → `/private/var/folders/...`）都会命中
  `/private` 前缀 ⇒ 这正是上一批 M68 实现后回退、探针 P-3 至今 SKIP 的成因，也是
  "省事路线代价 = 数十条断言变红"的成因。两条账**同一个根**。
- **出路三条**（等裁定，本轮一条不动）：
  - **a** 把 `/private` 收窄为真正要护的子集（`/private/etc` 一类），排除 `/private/var/folders/*`
    ⇒ 根侧解析真实路径判清单可以开，M68 残半与 M84 **一起解**。代价：清单语义变更，
    需另开一条 ID 记"清单条目粒度裁定"（本轮不预定号）。
  - **b** 清单不动，只在**根**上多算一次真实路径，且解析结果**新增命中时只警示不改判**
    ⇒ 不推翻任何既有断言，但 M84 的洞原样保留（等于把"知道但不动"写进代码）。
  - **c** 不做，手册补一句"不要用符号链接作为扫描根"当已知限制。

## 27. 九条裁定的落盘与第一批细化设计段（2026-09-22；用户逐条选定，非本代理推定）

### 27.0 裁定结果逐条入账

| 条目 | 用户裁定 | 本轮取证对上一行登记表述的更正 |
| --- | --- | --- |
| M82 | **统一成 KiB/MiB/GiB**（只改标签，数字不动）+ **不收归三份实现、只加防分叉钉** | 全仓三份格式化器两派命名；"改单位会让前端断言变红"不成立 |
| M79 | **判据归后端、文案归前端**（`noUndoTitle` 为唯一文案方，后端两处改吐机器可读码） | 两份文案已逐字漂移；`:68` 注释引用的是包装名 |
| M62+M85 | **按卷型分岔**（卷型已知按卷型，未知退现状默认，并显式给出"无从判定"） | 两条被点名断言钉的是不同分支，只有 `:57` 与修法相关 |
| M105 | **用户排除模式按卷敏感度过渡**（与"折叠判据按卷"同构） | 全仓关于此口径的文档声明确实为零；sysguard 第二条 `EqualFold` 在 `:254` |
| M48 | **做两处适用的，第三处写明残余窗口并另开 ID** | `claimExact` 对 undoHardlink 不适配；§6.9.8 更正的先例已由 M64 建立 |
| M56 | **undoMove 与 undoTrash 同形**（`claimExact` → `.fdd-restored`） | 该选项不是引入第三种约定，是补齐同一文件里已有的两条腿 |
| M84 | **收窄 `/private` 条目** + 开根侧真实路径判清单（M68 残半一起解） | 遍历从不跟随符号链接 ⇒ 队列双键并不需要，第二判据点只有根侧一处 |
| M91 | **A 内容级复核**（删/搬前按组比对） | 调用点真读数 46（生产 2 + 测试 44），登记的"约 15 处"过期 |
| M63 | **只翻分隔符腿 + 把钉子收窄到大小写腿** | `Fold` 现状无条件走 `pathnorm.Slash(p, "\\")`，冲突钉子仍在 |

**执行节奏（用户选定）**：分两批、先轻后重。第一批 = M82 / M48 / M56 / M84 / M63；
第二批 = M79 / M105 / M62+M85 / M91。每批内部照"设计段 → 实施 → 划账"三提交，每批完跑 15 行门禁。

### 27.1 第一批范围与顺序

按"能独立红、互不遮蔽"排：**M82**（纯标签 + 两条新钉，零判据变更，先落地建立单位基线）→
**M63**（`Fold` 分隔符腿，只动一处判据 + 收窄一条钉子）→ **M84**（清单条目 + 根侧真实路径，两条必须同批，
见 §27.5 的耦合说明）→ **M56**（`undoMove` 同形）→ **M48**（两处抢占 + 第三处注释）。

★ M84 内部有硬耦合，不许拆成两条交付：`/private` 不先收窄就开根侧解析 ⇒ darwin 上每个
`t.TempDir()` 根都被判"脱离系统保护"，正是上一批 M68 回退的现场（§26.7 补）。反过来，只收窄条目
而不开解析，`/etc` 型链接根依旧绕清单。**两条一起做才是一次完整的修法。**

### 27.2 M82 设计：只改标签，钉住"同一字节数在三处只有一种写法"

- 改点两处，都是标签不是算法：
  `frontend/src/utils/format.ts:3 humanBytes` 末行 `` `${(n/div).toFixed(1)} ${unit}B` `` → `` ... ${unit}iB ``；
  `cmd/fdd-cli/main.go:194 humanBytes` 末行 `"%.1f%cB"` → `"%.1f%ciB"`。`n < 1024` 的 `%dB` / `${n} B` 腿不动。
  `internal/ops/recycle_policy.go:18 humanSize` 本来就是 KiB 派 ⇒ **一字不动**。
- **防分叉钉的形状**（用户选"不收归只加钉"）：钉的是**同一输入对应同一字面输出**，不是 grep 全仓：
  1. Go 侧一条测试，把 `cmd/fdd-cli` 的 `humanBytes` 与 `internal/ops` 的 `humanSize` 对同一批尺寸
     （`1<<10`、`1<<20`、`1536`、`1<<30`）逐个比**单位后缀一致**（两份实现精度不同：`%.1f` 与 `%.2f`
     混用 ⇒ 只比后缀不比小数，避免把精度差异伪造成分叉）；
  2. 前端 `frontend/tests/` 新增一条，把 `humanBytes(1<<20)` 钉成字面量 `"1.0 MiB"` 一类。
  ★ 跨语言那一层（Go↔TS）实做到的是"**两侧各自钉住同一个字面量**"，不是运行期互查 ——
  两边都写 `MiB`，任何一侧改回 `MB` 自己红。这个边界必须写在划账里，不许写成"跨语言一致性已钉住"。
- 手册 `docs/09` 通篇 KiB/MiB ⇒ 改后无需动手册；`app_test.go:402` 钉的 `"20MB"` 是 **preview 体积上限的
  错误文案**（`app.go` 里手写字符串，不是格式化器输出），**不在本条改动面，也不该被顺手改**。

### 27.3 M63 设计：分隔符集合按平台注入，大小写腿不动

- 现状 `fscase.go:41-47 Fold(p, sensitive)` 无条件 `pathnorm.Slash(p, "\\")` ⇒ 非 Windows 上
  `a\b` 这个合法文件名里的反斜杠被当分隔符吃掉。修法：分隔符一侧按平台给（Windows 给 `"\\"`，
  darwin/linux 给空 ⇒ 需先确认 `pathnorm.Slash` 对空分隔符集合是恒等；若不是，走它已有的另一入口而不是改它）。
- **只翻分隔符腿**：`if sensitive { return q }; return strings.ToLower(q)` 这条大小写腿一字不动 ⇒
  非 Windows 上"同一目录的两种大小写拼写仍折成同键"这件事保持平台无关（那是 C1/M36 的地基）。
- ★ **改动面边界（约束 7）**：`visitKey` 与 `dedupeRoots` 各自的归一**不在本条**内。
  `visitKey` 注释（`scanner.go:226-231`）明写"只归一分隔符，不做大小写折叠"且 M36 特意**不折**大小写 ⇒
  它带 `\` 归一是**遍历键**的事，与 `Fold` 的**根去重键**不是同一个用途。本批只动 `Fold`，
  并在划账里如实写明"`Fold` 与 `visitKey` 的分隔符腿从此按平台分家"这一后果，让下次审查看得见。
- ★ **实施时本句被码推翻（2026-09-22）**：`visitKey` 早在 M64 起就注入 `string(filepath.Separator)`
  （`scanner.go:162-164`），"带 `\` 归一"的是改前的 **Fold 自己** ⇒ 分家的方向写反了。本批改完两者**同口径**
  （unix 上含字面反斜杠的根名，`dedupeRoots` 的折叠键与 `visitKey` 的遍历键从此一致），
  划账要写的是"收敛"而不是"分家"。仍按字面 `"\"` 归一的是 `sysguard.go:211-212/243`（跨平台保护清单）
  与 `filter.go:126/167/190`（用户手输模式），那两处按裁定不动。
- **钉子收窄（例外条款合法用法）**：`TestFoldSwapLegIsPlatformIndependent` 现钉"折叠腿整体刻意平台无关"。
  收窄成它该钉的那一半——**大小写腿平台无关**，并在断言旁写清"分隔符腿经 2026-09-22 裁定改为按平台
  （设计稿 §27.3），不在本钉子范围内"。**不删断言、不改期望值方向凑绿**；新增一条独立用例
  正向钉住"Windows 折 `\`、非 Windows 不折"。

### 27.4 M56 设计：`undoMove` 走 `undoTrash` 已有的两条腿

- 改点 `undo.go:191-208 undoMove`：删掉 `MoveFile(it.DestPath, filepath.Dir(it.OrigPath))` 的递增路线，
  改为 `claimExact(it.OrigPath)` 命中即原名回位、抢不到改落 `name + FddRestoreMark + ext`
  （与 `:148-151` 同一份 `FddRestoreMark` 常量，不新造标记名）。
- 跨卷腿：`undoMove` 现状不处理 EXDEV（`MoveFile` 内部处理）。改后必须自己带上跨卷回退，
  否则"原位空闲但跨卷"这一格从"能回"变成"回不去" ⇒ 直接复用 `undoTrash:157-185` 的
  `isCrossDevice` → `pathIdentity` → `copyVerifyFile` → `identityStill` → `removeSrc` 那一串，
  **一条防线都不许省**（那是 AS-H4 的成果）。
- ★ 落点契约写成文字进函数注释：**回撤成功 ≠ 回到 `OrigPath`**，成功返回的是**实际落点**；
  上层 `undo.go:204`（M113）已经在失败时也回传落点 ⇒ 本批顺带确认"成功路径的落点同样可信"。
- 取红：`undoMove` 可确定性构造（原位放一个第三方文件 → 现走 `MoveFile` 递增 ⇒ 实得 `photo.jpg` 的
  兄弟名 `photo-1.jpg`，而裁定要求 `.fdd-restored` 命名）⇒ 探针**不需要 seam**，直接红在名字上。

### 27.5 M84 设计：条目收窄 + 根侧解析（同批，判据只用于"问清单"）

- **条目改法**：删 `sysguard.go:145` 那条 `/private`，换成若干条真正要护的绝对路径条目
  （`/private/etc`、`/private/var/db`、`/private/var/log`、`/private/var/root`、`/private/var/spool`、
  `/private/var/at`、`/private/var/empty`、`/private/var/run`），**刻意不含** `/private/var/folders`
  与 `/private/tmp`（用户级临时区，正是上一批把普通根全误伤的来源）。每条带自己的 `why`，
  表注释补一句"祖先不再整片护，靠遍历期逐层剪枝命中后代"（`Dir` 用的是 `pathnorm.Under`，
  后代目录被单独问到时就命中 ⇒ 依据见 `sysguard.go:204-206` 那段"不逐段扫祖先"的既有口径）。
- **根侧解析**：只在 `scanner.go:239-243` 那一处"根自己是否命中清单"的判据上，额外问一次
  `EvalSymlinks(r)`；**解析结果只用来问清单，不进 `rawKeys`/`visitKey`/`rootsUnder`/emitted Path**
  ⇒ 遍历键与展示路径全部维持用户给的形，§26.7 补担心的"数十条断言红"从根上不发生。
  命中时把**用户给的那条形**记进 `UnprotectedRoots`（界面警示的是用户实际点的东西）。
- **两条负控制**（这一条的本体安全性全押在这两条上）：
  a) `t.TempDir()` 型根（真身在 `/private/var/folders/...`）不得进 `UnprotectedRoots`；
  b) 一个指向"清单内路径"的**符号链接根**必须进 `UnprotectedRoots`（本机不可用真 `/etc` 造，
     走 `guard` 注入：现有 `scanner.go:133 var guard` 已是可换装的包级变量，测试注入一条
     只命中链接目标绝对路径的假条目 ⇒ 目标命中而链接名不命中 ⇒ 钉住的正是解析那一步）。
- ★ **P-3 的账要一起结**：上一批因误伤回退、至今 SKIP 的那条探针（`TestWalkRootEscape*` 系，
  认锚点不认行号）本批必须转真跑。★ 转不了就明写"仍为 SKIP、不作通过"（AS-K2），不许默默删掉。

### 27.6 M48 设计：两处抢占 + 一处写明不可消除的残余窗口

- **`undoSymlink`**（`undo.go:324-425`，Remove 在 `:402` 附近）：`os.Remove(OrigPath)` 与"改名备份上位"
  之间插入 `claimExact(OrigPath)`；抢不到 ⇒ **不改名**，报"链接已删、原位又被占，备份仍在 `backup`"。
  抢到 ⇒ 改名替换的是我们自己的 0 字节占位，失败路径记得 `claim.release()`（与 `:159/:166/:171` 同形）。
- **`abandonForeignBackup`**（`merge_guard.go:112-117`）：把"Lstat 判空 → 改名"整体换成
  `claimExact(dup)` → 改名 → 抢不到就走现有那句"保持不动、只报位置"。**O_EXCL 本身就是一次问+一次占，
  原 Lstat 由此消失**（不是保留 Lstat 再叠一层 claim，那是双查不是抢占）。
- **第三处 `undoHardlink`**（`:216-323`，改名在 `:289`）：`claimExact` 不适配 —— 它核验的对象**正占着那个名字**。
  本批按裁定只把它写清楚：在 `:289` 上方补注释，点名"身份复核与改名之间存在窗口，
  第三方**针对本次操作**删我们的链接再落子时会被顶替"，并按 §27.8 新开一条 ID 登记（不预定号，
  实施时 grep 登记表取实际最大号 + 1，当前表内最大是 M151）。
- **取红为什么可行**（这是本条能不能开的关键，先写死）：两处都加一个**生产恒为空操作**的钩子
  （与 `fscase` 的 `fireProbeHook` 同一既有惯例），在"检查/抢占之后、改名之前"调用。
  改前树：钩子里让第三方**被动落子**建 `dup` → 我们的改名把它整个顶掉 → 断言"陌生人还在"当场红。
  改后树：名字已被我们的占位占住 → 钩子里那次创建撞 `EEXIST` 失败 → 陌生人根本没能落子 → 绿。
  ★ 这钉住的正是"同步盘/下载器被动落子"这一档现实威胁，而**不**声称钉住"专门删我们占位再抢"那一档
  （后者是 `undo.go:141-143` 已写明的不可消除残余窗口，两者不许混写成"竞态已消除"）。
- 变异计划：M22-a 把 `undoSymlink` 的 `claimExact` 退成"直接改名"；M22-b 把 `abandonForeignBackup`
  退成"Lstat + 改名"；两条都必须各自杀掉对应的钩子用例（逐条记包名表，按 §16 口径 1 做集合对账）。

### 27.7 本批门禁与用例计数计划

15 行门禁全套 + 用例计数重取（当前基线 **718** 条 Go、三平台闭包 **666/671/678**、前端 node 43 +
接线锚点 15）。本批预期新增：M82 两条（Go 1 + node 1）、M63 一条、M84 两条负控制、M56 一条、
M48 两条 + 一条防回归 ⇒ **只多不少**；任何一条计数下降都算异常，要当场查。
★ 每条生产改动的"改前必红"逐条留原始输出，**红的那一格必须与预测一致**才算数（§6.11 纪律）。

### 27.8 本批明确不做

- **第二批四条不碰**：M79 的接口/契约改法、M105 的判据穿透、M62/M85 的三态出口、M91 的签名变更。
  它们各自的改动面要单独一轮门禁，不和本批混在一个未提交状态里。
- **`Fold` 之外的分隔符归一不动**（§27.3 的约束 7 边界）。
- **三份字节格式化器不收归**（用户裁定）。
- **`undoHardlink` 不引入腾位改名新原语**（只写窗口 + 开 ID）。
- **`app_test.go:402` 的 `"20MB"` 文案不动**：它不是格式化器输出，改了就是把 §27.2 的边界抹掉。

### 27.9 第一批实施期真读数（追记，2026-09-22；划账见 04 §6.22，代码面 `20b7a68`…`00047c0`）

**一、设计段的行号全部按"猜实现体"写的，实施后须以锚点为准**（本表是 §27.4~§27.6 引用的
那几处坐标的**真读数**，不改写上面原文，只在此更正）：

| §27 原文写的坐标 | 实施后真锚（2026-09-22） | 性质 |
|---|---|---|
| `undo.go:324-425`（`undoSymlink`，Remove 在 `:402`） | `undoSymlink` 在 `:349`，`os.Remove(it.OrigPath)` 在 `:428` | M56 抽出 `restoreInPlace` 把整支推下去 |
| `undo.go:141-143`（"不可消除残余窗口"） | `:158-160`，`restoreInPlace` 在 `:146` | 同上，原坐标是搬家前的 `undoTrash` |
| `undo.go:159/:166/:171`（占位/release 三处） | 现为 `:161`（`claim = c`）+ `release` 各失败支 | 三处随抽函数合并 |
| `undo.go:216-323` / 改名在 `:289`（`undoHardlink`） | 函数在 `:231`，其改名是 `hardlinkRename(tmp, it.OrigPath)` 在 `:314`（M152 注释 `:300-313` 就在它上方）；`:443` 那条 `hardlinkRename(backup, …)` 属 **`undoSymlink`** | ★ 取数时看清：两个函数各有一条同名同形的改名，`:443` 不是本节说的那条 |
| `merge_guard.go:112-117`（`abandonForeignBackup`） | 函数在 `:116`，抢占后的改名在 `:120` | 一致（原区间正确） |

**二、三处预测被码推翻或收窄**（按纪律必须写下来，不许静默修正设计段口径）：

1. **§27.3 预判"分家"，实测是"收敛"**：设计段担心 `Fold` 注入宿主分隔符后与 `visitKey`
   的归一分家；真读数是 `visitKey` **早已**按平台走，本批反而把两者对齐了（已就地追记 §27.3）。
2. **§27.6 的"三处改走 `claimExact`"只有两处走得成**：`undoHardlink` 要占的名字正被**刚核验的
   对象自己**占着 ⇒ O_EXCL 必撞合法现状，问不出"现状正常"与"已被顶替"。这一处按裁定写成窗口并
   新登记 **M152**，且**不给它挂 `beforeClaimRename`**（挂了只会钉出一条必须失败的测试）。
3. **§27.5 的负控制 b) 换形**：设想跨包注入假 `eAbsPath` 条目，开码发现 `sysguard.table`/`entry`
   全未导出 ⇒ 需新增生产 API，按约束 7 不换形状。证据拆三处：P-3 名字腿 + 夹具自检 +
   本包新用例 `TestDarwinPrivateEntriesAreNarrowed`（读表断言"八条各带 why、整片 `/private` 不得回来"）。

**三、计数对账：与 §27.7 的预算逐条相符，且"只多不少"成立**

- 基线 718 → 现读 **725**（Go，全仓源码 grep 与 `go list ./...` 逐包相加两通道同值）＝ **+7**，
  逐项：M82 +1、M63 +1、M84 +1、M56 +1、M48 +3。M84 预算里那句"两条负控制**用例**"实际落成
  **一条新增 + 一条既有反向钉转真用**（P-3 由 `t.Skipf` 转真跑 ⇒ 门禁 top_SKIP 6→5），
  ★ 不是少了一条，是"负控制"这件事由既存的 P-3b 承担。
- 前端：node **43 → 45**（预算 1 条、实得 2：正例 + 防分叉钉），接线锚点 **15** 不变 ⇒
  合计 `45 + 15 = 60 项全部通过`。
- 平台闭包三值 **666/671/678 → 673/678/685**：三条腿**各 +7** ⇒ 本批七条全平台中立，
  四象限（Windows 35 / Linux 12 / darwin 17 / unix 11）一条没动。闭合式见 §3.2 第八次核对。

**四、门禁与验证**

- 全套 15 行（不带 `--fast`，日志 `/tmp/gates_b1_1.log`）：`rows=15 PASS=14 SKIP=1 FAIL=0`；
  唯一 SKIP 是第 15 行 `smoke-symlink` `rc=2`（本机 uid=501 挂不了独立文件系统），按 AS-K2 **不读作通过**。
  逐条实施当时各跑过 `--fast`：`PASS=11 SKIP=4 FAIL=0`。
- `go vet` linux/darwin/windows 三条腿全过、`gofmt -l` 空、`go test ./...` 23 包全 `ok`。
- **一次流程事故如实记**：还原变异 V-a 时用了 `git checkout -- internal/ops/undo.go`，把**未提交**的
  M48 改动一起冲掉了。因动手前 `cp` 过 `/tmp/m48/undo.go.ok`，`cp` 回来 + `diff` 逐字核对
  `RESTORE-IDENTICAL` 才继续，交付未受影响。纪律：**还原变异只能 `cp` 备份**，
  `git checkout --` 还原的是 HEAD 而不是"我改之前的工作区"。第二次还原（`merge_guard.go`）即改用 `cp`+`diff`。
- **一次"变异没落地"的假绿**（`b808e9f` 的 M23-a，详见 §6.22 二）：perl 改写 `Fold` 的注入实参时
  写出 Go 字面量 `"\\\\"`（**两个**反斜杠），变异体根本不是目标语义形状 ⇒ 当场"绿"。
  ★ 那次绿既不能记成"变异被杀掉"也不能记成"没杀掉"——它没落地，**读数无效**；改用精确写入重跑才拿到真红。
  假绿的形状和真绿一模一样，所以这一笔记在设计稿侧而不只记在划账里。

**五、本批未兑现（不得当作已通过）**

- CI 三条腿：本批未推送 ⇒ 无任何 CI 读数；windows/linux 真机行为只有本机 darwin +
  交叉编译的编译/vet 证据 ⇒ 按"代码已改、验证未兑现"记，**推不推由用户另行授权**。
- M63 的 Windows 宿主腿：`fold(..., "\\")` 三格在 darwin 真跑（H6 手法），但"宿主真是 Windows"
  时 `filepath.Separator` 那条守卫本机走的是 `if` 不成立一支 ⇒ 仍待真机。
...
★ **补记一（实施期，2026-09-22）：上面的落点清单漏了一条腿。** 它列的是"三处 `path.Match`
（`:245`/`:252`/`:269`）+ `matchSegs`"，但 `matchPath` 实际有**四条**判定腿，第四条是
**字面前缀腿**（无通配的 `"Cache/Sessions"`，走 `pathnorm.Under` 而不是 `path.Match`）。
按原清单改完，`path.Match` 那几处全绿、字面前缀那条一格都没放宽，而"含斜杠的目录前缀"
恰是用户最常写的排除形态 ⇒ 症状只解掉一半。测试因此按**四腿各一格**写
（`TestExcludeRelaxesAllFourMatchLegsOnlyOnProvenInsensitiveVolume`，不是 §28.3 起的那一个名字），
变异 M-M105-c 专门只退字面前缀腿，用来证明"漏一条腿"确实测得出来（真读数 §28.9）。

★ **补记二（实施期）：§28.3 只写了 `filter` 那一侧，`caseMode` 从哪儿来一句没写。**
这是本批真实的工作量所在，判据在此补齐：
- 值必须**按根**给，不能用一个全局读数。混合根（一个 NTFS 外置盘 + 一个 apfs 目录）时
  全局取"任一不敏感"会在敏感卷上放宽（多排除 = 少扫，方向安全但说谎），全局取"全部敏感"
  则外置盘那半 fail-open 原样留着 —— 两个都不是裁定要的"按卷过渡"。
- 传法：`relativeTo` 从返回 `string` 改成返回 `(rel string, rootIdx int)`（那条前缀循环
  本来就在找根，多返回一个下标零成本），调用点用 `rootIdx` 去取该根的 `fscase.Result`。
  ★ 兜底 `filepath.Base(full)` 那条给 `rootIdx = -1`，取不到根 ⇒ 传**零值 `Result{}`**，
  即"未确证"，不放宽。方向与 M62 同一条：证据缺失时不扩大排除范围。
- 按根的 verdict 不重新探测：`dedupeRoots` 早就算好并 accumulate 了 `keepVerdicts`
  （与 `kept` 同序，M62+M85 落地的），改成第四个返回值往外给即可。★ 这里**必须**保持同序，
  错位比不放宽更糟（会把 A 卷的语义按到 B 卷的根上），故给它一条专门的对序用例。
- **C1 的收窄**：`dedupeRoots` 今天单根直接提前返回、一次探测都不发（理由：不给最常用
  的那条路往用户目录写探测文件）。M105 之后这个"不探测"有了第二个代价 —— 单根用户的
  排除模式永远拿不到卷语义，fail-open 那一格在最常见形态上原样留着。⇒ 改成
  **`len(f.ExcludePaths) > 0` 时才为单根探测**（新入参 `probeSingleRoot`）。没有排除模式
  时探测结果没有任何消费方 ⇒ C1 的原口径继续成立，那条"单根零探测"的用例要跟着改成
  只钉"无排除模式 ⇒ 零探测"。
- ★ 连带改口径：`Result.CaseProbeUnproven` 的字段注释今天写着"单根一趟不发起探测"，
  C1 收窄后这句话会变成谎话（有排除模式的单根会发探测、探不到会计 1），必须同步改成
  "按消费方是否需要卷语义决定"。注释说谎比少一条注释更贵，这里没有"以后再对齐"的余地。
...
### 28.9 M105 实施真读数（2026-09-22，划账前的第一手证据）

**一、落地形状与对 §28.3 的三处偏离（偏离都是设计段的漏，不是实施的加）**

1. **落点清单少一条腿。** §28.3 写的是"三处 `path.Match` + `matchSegs`"，实际 `matchPath`
   有**四条**腿，第四条是**字面前缀腿**（`"Cache/Sessions"` 这类无通配前缀走 `pathnorm.Under`
   而不是 `path.Match`，`filter.go` 里 `pathnorm.Under(foldCase(...))` 那一行）。按原清单改完能让 §28.3 自己那条用例绿——因为它
   举的例子是 `Temp` / `TEMP/x.log`（段模式腿）。补了四腿各一格的表之后，变异 M-M105-c
   （只退字面前缀腿）立刻红在两格上，证明"按原清单改"会留下一半 fail-open。
2. **测试名与 §28.3 不同。** 设计稿起的名是 `TestExcludePatternIsInsensitiveOnlyOnProvenInsensitiveVolume`
   （单条、两格）；落地是 4 条：四腿放宽表（①/②/③ 三方向 × 4 腿 = 12 格）+ 剪枝腿 3 形态
   + 原样大小写负控制 + `path.Match` 只在 `matchFold` 里的静态防分叉钉。理由同 §28.8 那条
   （"分格钉"比"一条大用例"更能在红的时候直接指出哪条腿坏）。
3. **§28.3 完全没写 `caseMode` 从哪儿来**，而这是本条一半的工作量。判据已补进 §28.3 补记二
   （按根给 / `relativeTo` 回下标 / `dedupeRoots` 第四返回值与 kept 同序 / C1 按"有没有排除
   模式"收窄 / 兜底 `idx=-1` 折成"未确证"不放宽）。手册两条随之同时改：§3.1 的探测范围
   （原文"单根扫描既不写探测文件"在新口径下会变成谎话）与 §5.2 新增大小写口径一段。

**二、取红：行为级由 8 条变异提供（filter 4 + scanner 4），逐格读数**

| 变异 | 退化的那一格判据 | 落的红（原样抄自 `go test -v`） |
|---|---|---|
| M-M105-a `excludesRelaxed` 丢 `Proven` | "退默认"当"实测" | filter：放宽表 4 腿的 ② 格全红（"卷语义未确证却放宽了"）+ 剪枝 3 形态的"未确证卷上剪掉了" |
| M-M105-b 放宽整体失效 | fail-open 原样留着 | filter：4 腿的 ① 格 + 剪枝 3 形态的"目录剪枝腿没跟上"共 7 格红 |
| M-M105-c 只退字面前缀腿 | §28.3 落点清单的漏 | filter：**只有**"字面前缀"两格红（① 与剪枝）⇒ 腿级可分辨 |
| M-M105-d 两条件接反（敏感才放宽） | 负控制 | filter：4 腿的 ①+③ 双红、剪枝 3 形态的 ①+③ 双红 ⇒ 反向那一格真有牙 |
| M-M105-e `verdictFor` 恒给零值 | 根侧不放宽 | scanner：按根/剪枝/单根三条红、**对序那条绿**（它不经 `verdictFor`）⇒ 两半各自可测 |
| M-M105-f 退成全局取第 0 根 | "按卷过渡"退成"按某一卷过渡" | scanner：按根 + 剪枝红、单根绿、对序绿 ⇒ 全局读数只在混合根那一格露馅，正是 §28.3 补记二说的两种错法之一 |
| M-M105-g C1 不收窄（恒 `false`） | 单根永远拿不到读数 | scanner：**只有**单根那条红 |
| M-M105-h 第四返回值给成排序后全量 | kept 与 verdicts 错位 | scanner：**只有**对序那条红（`verdicts[1]` 取到被丢弃的 `sub` 的读数） |

★ 改前红是**编译级**的（`Apply`/`ExcludeDir` 换了签名，旧调用点不过不了），行为级的红全部由
上表提供——与 §28.8 第二条同一格口径，别把"改完就绿"读成"改前测过红"。

**三、一处按预定程序发生的断言范围收窄（不是"改断言让门禁绿"）**

`TestSingleRootDoesNotAskAndCountsZero`（scanner_m62_test.go）的注释**早就点名** M105 是那条
会把它逼红的诱因，并写明"红过之后要回来改口径"。本批正是那个时刻：改成
`TestSingleRootWithoutExcludesDoesNotAskAndCountsZero`，只钉"无排除模式 ⇒ 零探测"，反向那一格
（有排除模式 ⇒ 恰好探测）落在 `scanner_m105_test.go`。理由写在断言旁边，且两格合起来才是新口径
——单改测试名而不补反向格，就是把 C1 静默丢掉。既有 21 处 filter 测试调用点只是补参数
（一律传 `unproven`，语义逐字不变），没有一条断言被改。

**四、两次取数命令自撞（本批最该记下来的一条，属 §6.21 同族）**

1. 第一轮 4 条 filter 变异用 `-run M105` 过滤，而**测试名里根本没有 "M105" 这四个字**
   （文件名叫 `filter_m105_test.go`）⇒ `go test` 报 `ok ... [no tests to run]`，四行看着像"全绿
   即变异全被杀"。靠 `[no tests to run]` 这句话识破，换成 `-run TestExclude` 重跑才有上面那张表。
2. 变异脚本用 `cut -d'|' -f4` 取测试名列表，而**列表本身以 `|` 分隔** ⇒ 只取到第一条，
   M-M105-g/h 两轮报 `ok`（实为"只跑了那条不该红的用例"）。`cut -f4-` 修好后同一对变异的
   真实读数是"各红一条、其余绿"——**假 ok 的方向恰好是最危险的那种**（把没测说成测过）。
   两条都记在此处：门禁与变异的可信度取决于取数命令，不取决于它长得对不对。

**五、门禁与计数闭合（现读，`/tmp/gates_m105.txt`）**

- `rows=15 PASS=14 SKIP=1 FAIL=0` ⇒ 全绿；SKIP 行是 `smoke-symlink`（需要 root 挂载独立文件系统，
  AS-K2：只算未取读数，绝不当通过）。
- 第 6 行字段：`ok_pkgs=23`、`top_PASS=696`、`top_SKIP=5`、`sub_PASS=111`、`sub_SKIP=0`、
  `run=812`、`src_test=748`；两条 race 行 `data_race=0`（x2 全仓 23 包 / x4 根 1 包）。
- 闭合式三条全中：`top_PASS+top_SKIP=696+5=701=darwin`、`darwin+sub_PASS+sub_SKIP=701+111+0=812=run`、
  `go test -list '^Test' ./...` 报 **701** 与 darwin 通道同值。
- 平台通道现读 **696 / 701 / 708**（linux/darwin/windows，文件数 167 / 169 / 165）⇒ 每平台 **+8**、
  文件 **+2**；四象限（Windows 专属 35 / Linux 专属 12 / darwin 专属 17 / unix 专属 11）**一条没动**：
  `748−35−17=696`、`748−35−12=701`、`748−12−17−11=708` 三式全中 ⇒ "本批 8 条全平台中立"是量出来的。
- 子用例 +7 = 放宽表 4 腿 + 剪枝 3 形态（两条循环各一枚）。
- 前端腿一字未动（裁定③）：本批只碰 `internal/filter`、`internal/scanner`、两份文档。

**六、未兑现（不得当作已通过）**

- **真不敏感卷上的端到端没有读数。** 全部放宽格靠 `probeCaseVerdict`/`Compile` 注入 `Result` 值；
  本机拿不到 FAT/exFAT 真卷（`diskutil` 建卷要权限，CI 三条腿也没有）。⇒ "代码已改、真卷验证未兑现"。
- **Windows 腿的目录剪枝 + 排除放宽只有 CI 编译证据**（`vet-windows` 过），运行时按 §21 的先例
  仍属未取读数。
- `verdictFor` 的 `idx=-1` 兜底那一格在生产路径上不可达（`full` 恒由根拼出），只有单元级证据；
  留着它是因为 `relativeTo` 的 `filepath.Base` 兜底本来就在那儿，SCN-3（§15.0-A）教过这一格不能赌。
- 混合根的真实后果（外置盘 + 机内卷各一棵 `TEMP`）没有跨卷真机读数，只有两棵 `t.TempDir()`
  子目录的注入读数 ⇒ 按根传值的**形状**已钉，**盘上是否真有两种卷语义**属 fscase 那侧的未兑现面。

### 28.10 M91 实施真读数（2026-09-22，划账前的第一手证据）

**一、落地形状与对 §28.4 的两处偏离**

1. 落点、判据、三格处置与 §28.4 一致：`guardContent` 排在 `guardIdentity` **之后**、动作之前，
   delete / hardlink / symlink 各一处；`move`/`trash` 不接，只在两处留了"为什么不接"的注释。
   合并腿内部的次序是 dup 身份 → **dup 内容** → keep 源内容（M91 插在 keep 源校验之前，
   让"要销毁的那一份"先自证清白）。★ 文案一处偏离：§28.4 给的是「内容在扫描后被修改，已拦截
   （M91）」，落地是「动作前复核失败：文件在扫描后被修改，已拦截：盘上未做任何改动，请重新扫描」
   ——**必须带「动作前复核」这四个字**，否则与扫描期校验循环那句（`executor.go` 校验分支的
   「校验失败：文件在扫描后被修改」）同形，测试就分不出拦它的是哪一道；`assertM91Blocked`
   专门钉了这一条。"不新增状态字符串"守住（Stage 仍是 `verify`）。
2. 用例从 §28.5 预期的 **2 条**长成 **10 条**：三条腿各一条 + 变长那一格一条 + 另两格处置
   （已消失 / 读不动）各一条 + 负控制两条 + 防误伤一条 + 接线面静态计数一条。分格理由同
   §28.8 / §28.9（红的时候直接指出是哪条腿，而不是一句"合并流程坏了"）。

**二、取红：这一格取不了"删掉守卫"那种红，只能靠变异**

`beforeActContentRecheck` 这道缝**本身就是本批新增的** ⇒ 真正的"修前"形态是"缝不存在 ⇒ 钩子
无处落脚 ⇒ 改写压根没发生"，那种红落在前置自检（"钩子跑了 0 次"）上，不是落在数据丢失上。
第一次照 §28.4 试 M25-f 正是这个形状：三条腿红在 `assertM91Premise` 的「读不动 / 内容不是那份
新写入的字节」——那是**夹具在喊**，不是守卫在喊（前提检查读的是 Execute 之后的盘，把"守卫没了"
和"夹具没铺好"压成同一句话）。两处修正：

- 内容快照移进钩子里取（`recheckProbe.snapBytes`）⇒ 前提只看"改写当下"，Execute 之后的盘
  归 `assertM91Blocked` 判；
- 修前红全部由变异提供（保留钩子、退掉判据），与 §28.9 第二条同一条口径。

改后 M25-f 的红落在 `assertM91Blocked` 第一句，原样读数：
`被就地改写的 dup 仍然被处理了（OK=[…/dup1.bin]）：M91 的守卫没生效，那份新内容已经销毁`。

**三、变异真读数（两条，各杀各的）**

| 变异 | 退化的那一格 | 现读（`-run TestM91 -count=1 -v`） |
|---|---|---|
| M25-f `guardContent` 恒判通过（= 修前形态） | 身份过了就动手 | **5 红**：delete / hardlink / symlink / 变长 / 读不动；**5 绿**：已消失、move、trash、防误伤、接线计数 |
| M25-e 退成"只比 size 相等" | 把内容级复核做成元数据级复核 | **4 红**：等长三腿 + 读不动；★ **变长那一格绿** |

- ★ 第一次跑，两条变异红在**同一批格子、同一句断言**上（读起来像只做了一个变异）——那是
  "等长改写"这一格故意把 size 那一维排除掉的结果。补了 `TestM91DeleteBlocksSizeChangingRewrite`
  （改成 2048 字节）之后两条指纹分开：它专杀 M25-f、让 M25-e 过。这条在真码下也是真判据，
  不是为变异编的：编辑器追加一段、`dd` 补尾，size 都会变。
- ★ 「已消失」那一格**两条变异都不红**，如实记：它钉的是记账分格（Skipped 还是 Failed），
  不是这道复核的有无——M25-f 下 `os.Remove` 撞 ENOENT 仍然记 Skipped。这条不装作被变异杀过。
- 防误伤格（touch 过、内容没变 ⇒ 必须放行）两条变异下都绿，符合预期：它钉的是**假阳方向**
  （§28.4 末段警告的那个），与假阴互不覆盖。
- 还原两次均 `cp` 备份 + `diff -q`（`RESTORE-EXECUTOR-IDENTICAL` ×2），未用 `git checkout --`。

**四、代价读数（§28.4 承诺的那一条，换了通道，并已写明换的原因）**

§28.4 写的是"benchgen 造 N 个 dup + 一次 `fdd-cli delete` 前后耗时"——**这条取不到**：
`cmd/fdd-cli` 只有扫描通道（`main.go:79-93` 的 flag 全是扫描参数，没有 delete / execute 入口），
拿它量"删前复核"的成本是做不到的。改成包内 `BenchmarkM91ExtraContentRecheck`，直接量 M91 新增的
那一件事（同一个 dup 过一遍 / 过两遍 `VerifyFile`）。darwin / Apple M4 / `-benchtime=3x` / 32 MiB：

| arm | ns/op | MB/s | B/op | allocs/op |
|---|---|---|---|---|
| `VerifyOnce_修前` | 11,144,375 | 3010.89 | 1,809,517 | 5477 |
| `VerifyTwice_M91` | 22,928,806 | 2926.84 | 3,259,336 | 10928 |

⇒ 时间 **2.06 倍**、吞吐几乎不掉（3011 → 2927，差 2.8%）、分配 1.80 倍 ⇒ "总读盘约两倍"从注释
变成可复跑的数。外推：每个 dup 多读多算一遍 ≈ 32 MiB ÷ 2927 MB/s ≈ 11 ms ⇒ 清理 **100 GiB**
冗余约多花 **36 秒**（同吞吐外推，页缓存偏热 ⇒ 只当量级，且只是哈希成本、不含 IO 争用）。
★ bench 里不跑 `Execute` 的原因写在函数注释上：delete 会把文件删掉 ⇒ 每迭代重建 32 MiB，
写盘噪声盖过哈希噪声，两臂之差反而量不出来。

**五、门禁与计数闭合（现读 `/tmp/gates_m91b.txt`）**

- `rows=15 PASS=14 SKIP=1 FAIL=0` ⇒ 全绿；SKIP 仍是 `smoke-symlink`（需要 root 挂独立文件系统，
  AS-K2：只算未取读数）。
- 第 6 行字段：`ok_pkgs=23`、`top_PASS=706`、`top_SKIP=5`、`sub_PASS=111`、`sub_SKIP=0`、
  `run=822`、`src_test=758`；两条 race 行 `data_race=0`（x2 全仓 23 包 / x4 根 1 包）。
- 闭合三式全中：`706+5=711=darwin`、`711+111+0=822=run`、
  `go test -list '^Test' ./...` 报 **711** 与 darwin 通道同值。
- 平台通道现读 **706 / 711 / 718**（linux/darwin/windows，文件数 168 / 170 / 166）⇒ 每平台 **+10**、
  文件 **+1**；四象限（Windows 专属 35 / Linux 专属 12 / darwin 专属 17 / unix 专属 11）**一条没动**，
  三平台共有桶 673→**683**（+10 全落在中立桶）⇒ `758−35−17=706`、`758−35−12=711`、
  `758−12−17−11=718` 三式全中 ⇒ "本批 10 条全平台中立"是量出来的，不是断言的。
- 子用例 +0（本批没有新增 `t.Run` 循环）；前端腿一字未动（裁定③），M30 类型比对无新字段。

**六、未兑现（不得当作已通过）**

- **「删后建 + inode 还号」那一档仍然没有读数。** 本批钉的是**同 inode 就地改写**（APFS 上可复现）；
  还号要会回收 inode 号的卷，本机造不出来，CI 的 linux 腿本批未推（未获 push 授权）。内容级复核
  在判据上覆盖那一档（还来的号上内容必与组哈希不同 ⇒ Failed），但**没有为它写过一条断言**
  ⇒ 沿用 §6.17 口径："现象本机不可复现，内容级复核是它的兜底方向"，**不许**写成"M91 已消除"。
- **Windows 的「读不动」那一格造不出来**：`chmod 0` 在 Windows 只翻只读位、不拒绝读，
  `TestM91UnreadableDupIsUnverifiableNotModified` 自带这条前置自检并 `Skipf`（§21.3 W3 同一把尺子）
  ⇒ 该格只有 darwin 一条腿有读数；linux 腿在 CI 上到底跑还是跳**未取数**（runner 是否 root 未核）。
- **端到端 delete 耗时未测**：只有包内哈希成本的两臂读数，没有"扫描 → 勾选 → 删除"整条链的前后对比。
- **三平台运行时未验**：本批只到 `vet-linux` / `vet-windows` 的编译面；`guardContent` 走的
...
### 30.10 R5-F1~F6（Task E1）：文档六处错位——纯文档批，没有"改前红"可用

本项与前九项不同：**不动实现**，所以拿不到"改前红 / 变异杀掉"这类证据。能依托的自证只有两条，
本项把它们当作硬性要求执行：

1. **每条落点先开码取证**，把"报告说代码怎么做"与"代码实际怎么做"逐字对齐（下面 F1~F6 每条都带取证命令或 file:line）；
2. **收尾跑 `scripts/check-version-sync.sh` 作回归自检**。★ 但要把它的用处说明白：该脚本对 `docs/09`
   只校验一行（`:41` 取 「适用版本：」，即取值位 6），**碰不到**本批六条里的任何一条 ⇒
   跑它是为了证明"改手册没把版本位改坏"，**不是**本批六条的证据。

★ 一条必须先写下的层次事实：**全仓 15 行门禁里没有任何一条校验"文档内陈述与代码是否一致"**
（`run-gates.sh` 的 1~8 号是 gofmt / build / 三平台 vet / 两条 race，9~15 号是前端类型检查·构建·逻辑探针 /
version-sync / 三个冒烟脚本）。
本批六条错位里有五条是"门禁全绿而文档在说假话"，其中最硬的一条：§3.1 内联的 **516 Test**，
本项用 §3.2 那条指定命令实读 **783**——差了 267 条，而 `run-gates` 一路 14 PASS / 1 SKIP / 0 FAIL。
是否新增一条"文档计数门禁"（拿 §3.2 的 grep 与手册/04 的内联数字对账）属**新增门禁决策**，
本批不擅自做 → 交 E2 登记（低危，但它就是本批这类错位的复发来源）。

**F1（中）：09 的卷语义只写了两级，代码是三级**

- 报告：09:102 那条 bullet 写"探测结束立即删除，只读卷 / 无写权限 / 目录不存在时退回平台默认"，
  中间缺了 M85 的卷型档；09:277（M105 段）"探测不到而退回平台默认时**不放宽**"因此覆盖面写窄了。
- 开码取证（`internal/fscase/fscase.go:91-120`：`insensitiveVolumeTypes` 表在 `:101`、`fromVolumeType` 在 `:112`）：
  `fromVolumeType` 在探针不可用之前先问卷型，
  `insensitiveVolumeTypes = {msdos, fat, exfat, ntfs}` 命中即返回 `Result{Sensitive:false, Proven:true}`；
  不在表里才走 `defaulted()`（`Proven:false`）。**结论：报告成立**。
- ★ 一个不许混的方向：卷型档是**确证**（`Proven:true`），所以排除模式在那一格是**放宽**的
  （写 `Temp` 能挡住盘上的 `TEMP`）。手册里如果把它写成"探测不到也放宽"，等于把 M105 的
  安全边界反过来讲——所以 09:277 的改法只能是"探针**与卷型**都拿不到而退默认时不放宽"。
- ★ 一条边界不写进手册：linux 的 `vfat` / `msdosfs` **刻意不在**该表（§28.6 本批不做 linux 卷型腿），
  "插了 FAT U 盘的 linux"这一格走默认档 = 敏感。手册不提它，免得写一句实现里不存在的规则。
- 与 B2 的关系：计划要求"与 B2 的 09 补句合并成一次改动"。B2 那句（★ 这一圈探测**听取消**，R2-2）
  已在同一行内落盘，本项是在它**之后**追加第三级判据，不回改、不重复那句的取消语义。

**F2：09:323 保留目录的"分隔符不敏感"在 darwin/linux 上不成立**

- 取证链（`internal/ops/keep.go`）：`normalizeDirs:268` → `inAnyDir:285` → `inDirFold:323` 全程只做
  `filepath.Clean` + `fscase.Fold` + `pathnorm.Under`，**没有** `pathnorm.Slash(pat, "\\")` 那一步；
  而 `\`→`/` 被收在 `Fold` 内部（`:51-53` → `fold(p, sensitive, string(filepath.Separator))`，
  M63 起 sep 按平台取）。⇒ Windows 上 `Fold` 换分隔符（用户手写 `/` 与遍历给的 `\` 混用照旧覆盖），
  unix 上 `\` 是**合法文件名字符**、不换。**结论：报告成立，且给出的是机制不是猜测。**
- 落点：09:323 加分平台限定（分隔符腿仅 Windows 成立；macOS/Linux 上 `\` 是文件名里的普通字符），
  大小写腿不动（那条按卷实测，与宿主平台无关）。
- ★ 同一段 09:274 的排除模式那句（"分隔符统一按 `/` 处理，三平台一致"）**不改**：
  `filter.go:130/220/254` 三处都是 `pathnorm.Slash(..., "\\")` 字面常量、与宿主平台无关，那句是真的。
  两句读起来像自相矛盾（一处按平台、一处不按），差别在**判据对象不同**：保留目录比的是 Clean 过的
  绝对路径前缀，排除比的是相对路径段。本项只在手册里各自写实，**不合并判据、不改代码**
  （统一它们等于重开 M63 那条裁定，超出 E1 授权面）。

**F3：README 的可回撤范围少承诺了一项**

- 取证：`docs/09` 回撤表（:571 那一行）明写"软链接合并 | 支持"，且"链接已失效（悬空）时同样可回撤"；
  `internal/ops/undo.go` 的 `undoSymlink` 腿在本批 B1/C3 复核中读过（含崩溃残留自检）。README:16 只列了
  回收站 / 移动 / 硬链接合并。**结论：报告成立**，方向是**少承诺**（对外说支持的比实际少）——
  不伤安全，但 README 是对外第一屏，功能口径错在这一格会让人以为软链接合并不可恢复而不敢用。
- 落点：:16 补"软链接合并（跨卷）"；功能列表补一条，带上"保留源所在磁盘不能动，动了会悬空；
  悬空仍可回撤"这半句（只补能力不补前提，会引导出 09 §6.3.2 专门警告过的用法）。

**F4：附录 A 把两件不同的事勾在一起**

- 取证：`ls LICENSE*` → 根目录确有 `LICENSE`，首三行 `MIT License` / `Copyright (c) 2026 RunoobGo`；
  第三方声明文件（NOTICE / THIRD_PARTY 之类）**不存在**（`ls | grep -iE 'notice|third|credit|attribut'` 空）。
  04:5113 那条 `- [ ] LICENSE（MIT）与第三方声明文件` 因此是"一半已完成、一半空转"的合并勾。
- 落点：拆两个子项——LICENSE 勾 `[x]`（附取证），第三方声明留 `[ ]`。
- ★ 留空那格**不写"当前无第三方依赖"这类推测**：本应用静态链接 Go 运行库、前端产物内含 Vue 运行时，
  "要不要随发布物分发依赖清单"是发布决策，不属本批。本子项只登记事实：文件不存在。

**F5：两份 spec 的状态行与 `/private` 写法**

- (a) `2026-09-20-scenario-optimization-design.md:4` 仍写"已批准、未实施"，而 M6/M7/M8/M9 已按
  本稿逐批实施完毕。→ 改"已实施"，指针给**两份**（本稿 §1~§30 的逐条设计段与实施读数 + 04 §6 各轮划账），
  不抄结论、也不预定某个 §6.x 号（那编号还会随 E2 增长）。
- (b) `2026-09-21-m6-implementation-design.md:99`（§2 判据表）与 `:146`（§6 macOS 清单表）仍把
  `/private` 写成一条整片 `absPath`。实读 `internal/sysguard/sysguard.go:149-161`：M84（裁定见本稿 §27.5）
  已删整片、换成八条子树（`/private/etc`、`/private/var/{db,log,root,spool,at,empty,run}`），
  **刻意不含** `/private/var/folders` 与 `/private/tmp`。→ 两处各加 dated 括注指向 §27.5 与该文件行号，
  **表格原文一字不改**（它是设计阶段快照，抹掉等于毁"当时为什么这么定"的证据）。
- (c) 两份 spec 顶部补"历史存档 + 偏差不等于错"说明，对齐范本 `2026-09-20-cross-volume-symlink-design.md`
  的页首偏差表。★ 这一条其实是六条里唯一**对外承诺已被打破**的：README:72 写着
  "superpowers/ … 历史产物，顶部有偏差说明"，而 scenario / m6 两份顶部没有——错的不是 spec，是 README 那句。

**F6：04 §3.1 内联计数与 §3.2 现值分叉**

- 取证：`:93` 写"516 个 Test 函数 / 19 例逻辑探针"、`:98` 写"3 个 Benchmark"；同节下方 §3.2（`:171`）
  写 **758 Test / 4 Benchmark**。本项用 §3.2 自己指定的那条命令（含两个 `--exclude-dir`）实读
  **783 Test / 4 Benchmark**（2026-09-23）。⇒ **计划里"改指向 §3.2 现行真值（758/4）"这句本身也已过期**，
  §3.2 的 758 是第八次核对的快照。
- 落点（与计划略有不同，理由如下）：`:93` / `:98` **不抄任何新数字**，只改为"计数是当日快照 +
  真值以 §3.2（Go）与 `scripts/test-frontend-logic.sh` 收尾行（前端 node + 接线锚）为准"。
  ★ 修文档错位的最小正确改法是**去重（加指针）**，不是**再抄一份新数字**——抄新数字等于亲手制造下一个错位点，
  本批 §3.2 自己就是证据（第八次核对的读数在第五次核对的表下方仅一天）。真实数字的回填交 E2 第十次核对统一做。

**本项之后仍不成立的声明（未兑现面）**

- 本批六条全是文档改动，**没有任何一条带行为级红证**：其正确性只到"取证命令与 file:line 对得上"这一层，
  读者复核方式是把上面每条的取证命令再跑一遍。
...
### 30.14 Task E2（收口批）：配号规则、第十次核对的计数通道、以及"哪些格子本批不许写绿"

**先记一处落点错位（计划与本稿的号不是一套）**：计划 Task E2 Step 1 写的是"docs/04 新增 §6.25"，
而 §6.25 已被第 6 批（第四轮修订推送后的 CI 首红复批，设计段 §31）占用 ⇒ **E2 的划账节落 §6.26**。
★ 不采取"把 §6.25 让给 E2、复批挪号"那种改法：划账节的号是**写它的时刻**决定的，事后挪号会让
§6.25 里"⇒ 划账 §6.25"那类交叉引用集体失效，而集体失效的交叉引用比一个跳号的标题贵得多。

**配号规则（落笔时预定 34 行 M156~M189；实交付 **35 行 M156~M190**，多出的一行是复核过程中
查出的 M190 DOC-2——"不预定区间"这条规则就是为这种情形立的）**

1. **复核通过才配号**：一条 R 项要拿到正式 M 号，行内必须说得出「RED 用例名 / 变异读数 / 未兑现清单」
   三件；说不出第二件的（纯文档批 E1、门禁批 D2 的部分格）在行内明写它取证的是哪一手，不假装有变异。
2. **登记不修也配号**：J-6 裁定"其余逐条登记"⇒ 欠账进表本身就是它的交付面，配号不代表开工。
3. **只增行**：登记表使用约束第 (1) 条本批依然成立 ⇒ 已实施项**不回去改** M91/M105 那一类既有行。
   R1-1 补的正是 M91 那一族，也另起 M156 一行，靠 §6.26 一的 R→M 对照表回连；★ 同理，登记表内
   `grep -c 'R1-1'` 为零不是缺口，对照表才是那一族的可追溯入口。
4. ★ 编号差池的旧例（M153 那一行自记的"拟号不等于分配"）本批不复现：号在落笔时现取，不预定区间。

**第十次计数核对的通道**（与第八、九次同一把尺子，另加一条本批独有的）

- 全仓指定命令（带那两个 `--exclude-dir`）= **787**；裸命令 = **804**，差的 17 条全在 `.workbuddy`（现读确认一字未变）。
- 逐包相加（`go list -f '{{.Dir}}' ./...` → 每包 `grep -c '^func Test'`）= **787** = 上一条 ⇒ 无缺包（第五次核对那个病）。
- **★ 按提交对账（新增的一格）**：本批动了 `*_test.go` 的 8 个实施提交逐个
  `git show --unified=0 <sha> -- '*_test.go' | grep -c '^+func Test'` ⇒ 2+5+1+7+2+8+3+1 = **29**，
  与 787 − 758 = **+29** 逐字闭合；包级差额 ops +8 / scanner +6 / filter +5 / fscase +5 / 根包 +5 = 29，
  与逐包通道现读值逐格对上。★ 这一条比"两个总数相等"强：它同时排除了"表里某个包被顺手抄错"
  与"增量里混进别人未登记的提交"。它还独立复算出 E1 那一句"本项实读 783"为真（787 − F1 的 3 − M155 的 1）。
- 平台三值 **735 / 740 / 747** 由四象限闭合式反查：`787−35−17=735`、`787−35−12=740`、`787−12−17−11=747`
  三条全等 ⇒ 本批 29 条**全是平台中立**（四象限一字未动，故不重举 tag 做集合运算）。
- darwin 那一行另有第二条独立通道：本机 `go test -list '^Test' ./...` 实数 **740** = `GOOS=darwin` 的 `go list` 通道，
  且 `no test files` 包数 **0**。
- 前端腿（§3.2 明令不并进 Go 数）：node **55 → 78**（+23，四个新 `.test.ts`）、接线断言 **17 → 24**
  （净 +7：新增 8 行、收紧删掉 1 行旧的松锚，其中 `wiring_count` 是新辅助函数），脚本自报合计 **102**；
  ★ 表内正文那一格改前写的是 15，而改前现读是 17 ⇒ "第九次核对没重取前端腿"，如实记下而不是把 15 直接改成 24。
- 跨卷 harness：`smoke-symlink-assert` 自报**走到断言 18 条（下限 15 条）**，`MIN_CHECKS` 照旧不抬。

**哪些格子本批不许写绿**（写进 §6.26 五，不等别人来问）

- CI 三条腿在 `95cedc7` 之后的读数**未取到**（本批默认不 push）⇒ "CI 已绿"整批不许写，§6.25「六、复跑真读数」仍留空。
- D1 / D3（build.yml 加固、CI 腿 `smoke-symlink` rc=2 判红）按用户"等我手动改完再说"未动
  ⇒ J-1、J-2 两格在本批交付面里**不存在**，不许被读成"已裁已做"。
- A2 的 `case_pair=false` linux 判红支、B1/B2 的 Windows 腿、C1/C2/C3 的浏览器实测、D2 的 ①~⑦ rc=0 全绿：
  一律按"代码已改、验证未兑现"记。
- 一条未解释的历史红（`TestSupersededScanWritesNoHistoryRow`）与一次未复现的加压红
  （`TestVerdictCtxHonorsDeadline`）各配一个观察项行（M186）/挂账（M155 行内），**不划成已修**。
