...
- `os.MkdirTemp` 自建目录 + 自注册 `t.Cleanup`：对 `os.RemoveAll` 做**有界重试**
  （每 10ms 一次、上限 3s；到顶仍失败才 `t.Errorf`）。修形实为**两段**：拍平目录
  （`t.TempDir()` 是 `<T>/名/001` 两层，清理窗更宽）+ 重试兜残窗——§3-3 的读数显示
  前者才是红率的承重段，后者是按机制立的保险。
...
2. **修后必绿**：同一施压命令（提到 `-count=200`×6 加大覆盖面）**2400 次跑零红**；
   单进程 `go test -race -count=2 ./internal/fscase` 全绿；`gofmt` 零文件。
3. **变异验证（落盘后按真实读数补正——分两层，不合并成一句"变异必红"）**：
   - **形状层，变异必红：成立**。把两条用例的夹具换回 `t.TempDir()`（其余一字不动），
     修前同压口径 `6 路 × -race × -count=100` 立刻复红：**5/6 进程红、共 10 条**，
     错误行与 CI 逐字同签名（`TempDir RemoveAll cleanup: … directory not empty`）。
     ⇒ 机制归因（迟到探测 × 框架清理同窗）由这一格钉死，不是推断。
   - **重试层，单摘重试不能稳定逼红（如实记）**。保留拍平形状、只把清理改回单次
     `RemoveAll`：6×100、8×400（另加 2 路旁路负载）与逐字复刻修前命令的 6×100
     共 **5600+ 次跑 0 红**。解释：红率对"清理遍历目录的窗口宽度"敏感（t.TempDir 是
     `<T>/名/001` 两层，readdir→rmdir 之间隔更多 syscall；拍平后窗口窄到施压也难撞）。
     ⇒ 重试环的定位是**兜住任意残余窗口的论证件**（"RemoveAll 成功一次即窗口已过的机器
     证明"，§2），其必要性不冒领"被变异单独钉死"的读数；总绿读数（修后 2400+ 零红）
     按"形状+重试"合并修形记账。
