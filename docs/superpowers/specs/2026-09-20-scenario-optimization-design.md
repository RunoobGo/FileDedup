...
> **历史设计存档（2026-09-20 批准 / 2026-09-21 起逐批实施）**：本文是**调研 + 方案总纲**的规格快照，
> 不再随代码演进回填。每个 P0 项实施前各自展开的细化设计、以及逐条**实施读数**（含"设计段的预判被开码
> 推翻"就地打 ★ 追记的那些）在 `2026-09-21-m6-implementation-design.md` §1~§30，台账划账在
> `docs/04-开发与测试计划.md` §6 各轮。因此**本文与当前实现存在偏差**，阅读时以那两份与代码为准。
> 本轮（第四轮全仓审查 E1）实读并在此点名一处：§2.4 的 macOS 初版清单把 `/private` 列为一条（整片锚定），
> 实施时按 M84 收窄为八条后代子树（`internal/sysguard/sysguard.go:149-161`，裁定记录在
> 细化稿 §27.5）。★ 这一条是"抽查到并核实"的，**不代表其余内容都无偏差**。

- 日期：2026-09-20
- 状态：**已实施**（本文只留调研与判据；实施回执见上面两份）
...
> **实施回执（2026-09-21，划账见 04 §6.9.6；本节原文按登记当时保留不改）**：
> 第一项（fscase 单根）已随 M6 交付，但**"修法已定（无条件逐根探测）"这句被动手前取证
> 推翻**——探测只能描述**根本身所在的卷**，而"不敏感目录的子树里嵌着一棵大小写敏感卷"
> 这个构型（本机 `hdiutil` 挂进普通目录即可造出）探出来仍是"不敏感"，照样丢子树。实际
> 修法改为**单根不做折叠**：单根出发的遍历里每个目录只会以唯一拼写出现（根按原样入队、
> 其后路径一律由父 + ReadDir 原名拼出、链接/junction 已跳过），折叠没有可并之物，只剩
> "把两棵不同目录误并成一棵"这一种作用。≥2 根的同类缺口本轮未修 → 登记 **M36**。
...
> **实施回执（2026-09-21，划账见 04 §6.9.5；本节原文按登记当时保留不改）**：
> 交付面与本节原文**一处收紧、一处更精确**，其余照做：
>
> 1. **接入点收紧**。本节写"`ops/verify.go` 执行前"，实测（设计稿 §5.0 E9 读码）
>    更精确的位置是**执行器校验循环 `default:` 分支**——五种 `op.Kind`
>    （`trash`/`delete`/`move`/`hardlink`/`symlink`）**全部**只处理进了 `toProcess` 的项，
>    而 `toProcess` 唯一来源就是那道循环。守卫加在这一处即覆盖全部破坏性动作，
>    不必（也不该）在 `verify.go` 里加——那里是"内容对不对"，这里是"动了会不会丢东西"。
>    该结论由变异 M-P3-g（守卫只覆盖 `trash`）钉住：`delete`/`move`/`hardlink`/`symlink`
>    四格全红而 `trash` 格绿。
> 2. **判据比"任意非 `:$DATA` 即认定"更细**。`::$DATA` 之外还有 `:$DATA` 这种省略空名段的
>    等价写法，且型别段大小写不敏感；把它们判成命名流的代价是**整卷每个文件都被拒**。
>    故 `StreamIsNonDefault` 先排除三种默认写法，再对无法解析的畸形串**不判**（宁可漏判
>    一个畸形势，也不误伤默认流）。失败方向的取舍另有一张 fail-closed 表：
>    `ErrDenied`/`ErrOther` → 拒绝（同 `verify.go:104-108` 的"宁可拦一次让用户重扫"），
>    `ErrMissing`/`ErrNoStreams`/`ErrFSNoStreams` → 放行。
> 3. **命名流不进身份判据**（本节未涉及，实施时明确）：本项只在动手前拦一道，
>    两个"默认流相同、命名流不同"的文件**仍会被报成重复组**（清理被拒，但不是没被报重复）。
>    → 登记 04 §6.8.8 **M35**。
> 4. Windows 真机整条腿（胶水行为 + E7 结构偏移 + 两条真机用例 + 枚举开销）**未兑现**：
>    造夹具本身也需要 NTFS → 登记 **M32**，与 §2.1 的同族缺口并列。
