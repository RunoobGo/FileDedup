...
> | 6 | 2026-09-19 | main 首跑 CI 复盘：Windows FileKey 字段接线错误 + 四条环境假设 | 已修；Windows「组数偏少」家族待下一轮 CI 定性 |
...
> 与随后的修复工作树。
> **状态**：一处产品缺陷（Windows FileKey 字段接线错误）与四处测试缺陷已修，本机全绿；
> Windows「组数偏少」三条**只修到"能自证"的程度**，定性要等下一轮 CI（本机无 Windows runner）。
