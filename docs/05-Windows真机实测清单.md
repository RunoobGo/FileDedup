...
> 定位：本文是 **M7（平台证据包）** 的执行清单（04 :2987/:3754 登记的"下一批开工项"）。
...
1. **CI 有 windows 真机腿，但日志分不清"真跑"与"整包 Skip"**：`scripts/test-windows-quarantine.sh:73` 的 `go test -count=1 ./...` 三条腿都不带 `-v`（04 :2916 M100、:4215、:5050 三处自认）。因此凡标注"CI windows 腿已绿"的项目，其 Skip 出口项一律按"无读数"处理。
...
| # | 事项 | 落点 |
|---|---|---|
| A1 | windows 腿补一次带 `-v` 的跑法，把 `--- SKIP` 计数纳入读数 | `test-windows-quarantine.sh:72-73`（M100 遗留动作）→ 直接产出 V8b、`fsid_path`、`trash_windows_recycle` 等 12 条"绿但不知是否真跑"用例的差分 |
| A2 | 修 `undo_i6_test.go:136-138` 的**假前提 Skip**（注释称"Windows inode 身份未解析"，已被 fsid I7 推翻）→ 让 undoHardlink 强校验路径在 windows 腿重新暴露 | 域 E |
| A3 | 前端 pathpolicy 混例（盘符 / UNC / 尾分隔符 / 相对路径）是 node 用例，不必真机 | `plans/2026-09-22-full-review-revision.md:277`（AS-H6·D-1） |
...
- **杀软现实性**：至少一轮在**带第三方 AV / Defender 实时防护开启**的机器上跑（04 :824 点名的"北信源类"干扰面：句柄占用、Minifilter 卷重定向、回收站目录接管）。
...
| # | 场景 | 操作方式 | 结果验证（判据） |
|---|---|---|---|
| W1-1 | 正常入站 | 对 NTFS 卷上一批小文件（含中文名、含 >1000 项混合列表）执行"移入回收站" | ① 源路径消失；② 文件出现在**该卷** `$Recycle.Bin`（系统回收站可见、可"还原"回原位）；③ 应用历史里 kind=trash、`DestPath` 恒空（结构性缺失，记录读数即可）、徽标/title 走 `undoableFor(trash,windows)`＝不可应用内回撤；④ 三判据事后复核（源不在 + 该卷条目增量==预期 + 入站时间基线）全部通过，无"疑似已被直接删除"误报 |
| W1-2 | 超配额拒绝 | 把测试卷回收站属性容量设为 **1MB**，对单个 >1MB 文件执行移入回收站 | 见**含容量数字的拒绝提示**（`capacityReason`），文件仍在原位；不得出现"报成功但文件消失" |
| W1-3 | 批量部分超限 | 混合"能进 / 不能进"的多文件一批执行 | **整批拒绝**（`recyclableReason` 预检语义，`trash_windows.go:237-281`）；若走 executor 逐个回退，被 Shell 静默丢弃的条目必须记 Failed「疑似已被直接删除」而非 Skipped（`executor.go:587-630`，此语义 Windows 独有、至今零真机观测） |
...
| W1-7 | SHQueryRecycleBinW / 卷 GUID | 让带 `-v` 的 windows 腿（或本机 `go test ./internal/ops -run Recycle -v`，需 GTest 环境）真跑 `trash_windows_recycle_test.go` | 四条可 Skip 用例（GUID 43-59 / 配额 72-97 / SHQuery 104-123 / Nuke 三态 209-250）**逐条报 PASS 而非 SKIP**；`shQueryRBInfoSize==20` 布局钉在真 Shell 上语义正确 |
| W1-8 | 长路径回收站 | >260 深度文件执行移入回收站 | `SHFileOperationW` 不接受 `\\?\`（:255）且不在 longPathAware 解除清单（§6.6.4:957-961）→ 预期**显式失败并保留源文件**，不得静默永久删除 |
| W1-9 | Shell 静默降级 | 沙箱/被安全软件接管 `$Recycle.Bin` 的环境（或第三方清理工具在场）执行 | Shell 返回 0 但未进站时，事后复核必须报"疑似已被直接删除"——这条防线自身是本清单的被测对象 |

---

## P1 · 长路径链路（T04 / §3.5#4 / §6.6.4 / M33）

manifest `longPathAware` 已声明但**注册表前提、双条件生效、进程缓存**全部未复测（`build/windows/wails.exe.manifest:9-22`）。
...
`CreateSymbolicLinkW` 两段式与错误码翻译（1314/87/50/5）至今只过 `go vet`，"未经真实调用"（`quarantine:44-46`、开放项 E）。

| # | 场景 | 操作方式 | 结果验证 |
|---|---|---|---|
| W3-1 | 提权 + 跨卷 | 管理员运行应用，对两个 NTFS 卷的重复内容执行软链接合并 | 成功；`VolumeResolved` 为真、前端显示跨卷入口（`app.go:1046-1069`） |
...
`isCrossDeviceExtra` 认 error 17 的分支**没有一条用例真跨卷 rename 过**；本机（macOS）跨卷腿挂不上独立卷（04 :4035）。

| # | 场景 | 操作方式 | 结果验证 |
|---|---|---|---|
| W4-1 | 跨卷 move 一次 | 两物理卷间对 ~200 个文件执行移动（`executor_symlink_test.go:274` 点名的欠账） | 走"复制+校验+identityStill+删源"腿；字节/条目记账正确（M40 假账族核账）；不出现"源没删/目标缺" |
...
| W4-4 | 盘符≠卷 | `SUBST X: D:\somefolder` 后跨该"卷"操作；同一目录先后挂两个不同卷 | 记录盘符键分裂/混池的实际症状（`app_volume_test.go:15` 登记的顾虑）；回收站判据 2 是否按 GUID 而非盘符正确归卷 |
...
| W5-9 | **WIN-1（M72）补码后复测** | 当前代码未 `SetLastError(0)` → 先修（标✗"开工即须写"，04 :2968），再复测 W5-8 | 干净 lastError 下不误读 0xFFFFFFFF |
| W5-10 | 占位/缓存第③重证据 | 同文件两种拼写路径（`FILE.TXT`/`file.txt`）重复扫描 + 缓存命中 | 记录 `CtimeNs==0` 是否真机普遍（04 :575）、缓存主键两行的实际后果（域 O） |
...
| W6-5 | M35 症状面 | 两个默认流相同、命名流不同的文件 | 当前设计**仍报重复组但动作被拒**——确认界面表现与 09:520 口径一致，用户可读 |
...
| # | 场景 | 操作方式 | 结果验证 |
|---|---|---|---|
| W8-1 | 盘根扫描保护 | 以 `C:\` 为根扫描 | `pagefile.sys`/`hiberfil.sys`/`swapfile.sys`/`DumpStack.log.tmp`/`$Recycle.Bin`/"System Volume Information"/`WindowsApps` 全部被挡；`WindowsApps` 特权失败产生的 FailedItem 噪声量可接受（域 L）；**ExcludePaths 在盘根下不静默失效**（`rootprefix_probe_test.go:51` 自认的未兑现边界） |
...
| W8-7 | `$I`/`$R` 元数据 | 盘根扫描时回收站元数据文件 | 不误入分组（spec :318-319 登记项） |
...
| # | 场景 | 操作方式 | 结果验证 |
|---|---|---|---|
| W9-1 | WebView2 真运行时 | 在 Wails 窗口内（非浏览器 dev）验证：拖拽入库、事件流刷新、原生目录选择对话框 | 三项全部可用（`wails.ts:361-375` 明示浏览器不等价） |
| W9-2 | 不可回撤文案 | trash 一条记录，看徽标/title/toast | `undo-code-windows-trash` 文案渲染正确、裸码不漏出（`scan-undo-error` 契约）；"打开系统回收站"落到当前用户 SID 的回收站 |
| W9-3 | M8 呈现位 | 结果页横幅（`unprotectedRoots`）、保留名计数（`protectedFiles`） | 09:44 承诺"当前界面不呈现"是否仍未呈现；若借本轮开显示，同步改文档 |
...
| W9-8 | O_EXCL 撞目录 | `claimDst` 抢占目标为"已存在目录"时 | 记录真机返回 EEXIST(183) 还是 EACCES(5)（spec :1124-1127：判定无行为差异，读数回填注释即可） |
| W9-9 | 启动失败痕迹 | 故意制造启动失败（db 损坏等），GUI 子系统下运行 | `main.go:46-50` 的 stderr 在无控制台构建=丢弃 → 确认真机上是否留下任何可诊断痕迹，不留则立发布风险账 |
...
| W10-2 | 安装/卸载（#9） | 若引入安装器则全链路；当前为裸 exe——至少验证数据目录残留与"卸载"=删目录的行为 | 记录残留清单；同时裁定 04 :556 开放项"发布说明是否需前置强调不可回撤面" |
...
- 每条读数按来源编号回填 04（M7 总账 ⊃ E 组回收站/软链接、T04、M26/M29/M31/M32/M33/M35/M36/M45/M72/M100、§3.5 #2/#4/#6/#7/#9、AS-R4、:575、:1058、:1127）。
