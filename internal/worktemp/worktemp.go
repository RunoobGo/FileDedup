...
func IsTempName(name string) bool {
	...
	for _, f := range forms {
		...
		case kindInserted:
			// 插入式：标记之后只认三种形态——空串、以 "." 开头、`_<纯数字>` 再接任意尾巴。
			// 只认这三种是为了不放宽成 Contains（见上方 M1 说明）。
			//
			// ★ M147（04 §6.11 WT-1，第 3 轮）：这里原先写的是"名尾 / .扩展名 /
			// _序号.扩展名"。**"扩展名"那两个字超出了实现**：isExtStart 的判据是
			// `rest[0] == '.'`（:159），尾巴里还有几个点它都不管，于是
			// `photo.fdd-restored.anything.jpg` 也被认领成本应用的暂存并跳过，
			// 而生成侧（restoreMeta）只在标记前插**一个**扩展名，从不产出这种名字。
			// 本轮**不改行为**，理由是不对称的代价：判宽 = 替用户多忽略一个他恰好
			// 这么命名的文件（漏扫，会在结果里以"这个目录没扫到"的形式被发现）；
			// 判窄 = 把自己的 .fdd-restored 暂存当真实数据参与去重（M1 那一族，
			// 会往下走到删除）。真要收窄属独立改动，需连带补 `a.fdd-restored.x.y.jpg`
			// 的正反两条夹具。
			// 顺带一句真读数：`rest == "" ||` 这一支被 isExtStart 的第一个条件完全
			// 覆盖，写在这里只是把"名尾也算"说出口，删与不删行为一致。
