...
//
// 形状与 M86（undo.go:182）完全同型：跨卷复制已到家、删源失败 ⇒ 盘上是两份，
// 而 move.go:85 改前只说"两份并存"，一个落点都不给。
//
// 比 undo 侧更要紧的一格（本批取证新证）：改前的 `executor.go` move 分支
// （:622-630）拿到 `MoveFile` 的 `(dst, err)` 后，`err != nil` 就只记 `ocFailed` **并把 dst 丢掉**
