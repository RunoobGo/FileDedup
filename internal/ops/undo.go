...
//
// 防线，与 undoHardlink 的三重防线对齐但**判据不同**：
//
//	① OrigPath 必须仍是"指向 LinkSrc 的符号链接"
//	   （用 verifySymlinked —— 软链接自身身份无意义，必须解析目标再比）
//	② 备份（OrigPath + .fdd-old）必须存在
//	②b 备份内容必须全量 BLAKE3 等于 it.Hash（B3，R-操作-1；零值哈希跳过）
//	③ 删链接 → 还原备份；任一步失败都不破坏现有数据
//
// 与 undoHardlink 的关键差异：OrigPath 位置上的链接是独立对象，它的内容就是
// "目标路径"这个字符串，数据本体一直在 LinkSrc 处。因此这里**不需要**从 LinkSrc
// 复制内容回来，也就没有"复制源被篡改"的风险面——直接改名备份即可。
//
// ★ 但备份**不是**"内容天然正确"（B3 修正前的旧注释如此宣称，是错的）：.fdd-old
// 名被扫描器忽略，第三方能在合并后、回撤前把备份顶替掉，而 ② 只查存在性查不出来。
// 故补 ②b 与 undoHardlink 防线② 对称——还原前对备份做全量 BLAKE3 复核，不符即拦截、
// 链接与备份均未动，绝不把陌生文件当原文件还原回用户眼皮底下。
//
// ① 这一层同样不能省：用户可能在合并后把 OrigPath 位置上的链接删掉、换成自己的
// 另一个文件。若不做校验就直接删 + 改名，就会把那个第三方文件删掉、把备份顶上去——
// 用户会发现自己放进去的东西消失了。这是 R5（回撤时误删第三方文件）的具体形态。
//
// 悬空链接（保留项已被删/移动）**仍可回撤**：数据在备份里，与链接是否有效无关。
// 这种情况反而是最需要回撤的（用户想恢复原文件）。
func undoSymlink(it UndoItem) (string, error) {
	...
	// ②b 备份还必须**内容正确**（B3，R-操作-1）。.fdd-old 名被扫描器按
	// worktemp.IsTempName 忽略，第三方顶替备份对用户不可见；此前本函数只查备份
	// 存在性（上面那道 Lstat），与 undoHardlink 防线②（copy 后全量 BLAKE3 对
	// it.Hash）不对称。若只查存在就删链接 + 改名，会把顶替者当原文件还原回 OrigPath
	// 并 applyMtime 拨回记录时间——用户原数据已被顶替覆盖，眼下又多一个冒名文件。
	// 故还原前对备份做全量 BLAKE3 复核，不符即拦截、链接与备份均未动。
	// it.Hash 为零值（升级前旧账本无内容证据）时**跳过而非拦死**，与 undoHardlink
	// 防线②、undoSourceCheck、alreadyRestored 同判据（见 UndoItem 注释）。
	if it.Hash != ([32]byte{}) {
		h, herr := hashFile(backup)
		if herr != nil {
			return "", fmt.Errorf("无法读取备份 %s 以复核内容，为避免还原出错误文件已拦截"+
				"（链接与备份均未动）: %w", backup, herr)
		}
		if h != it.Hash {
			return "", fmt.Errorf("备份 %s 的内容与记录哈希不符（疑似被第三方顶替），"+
				"为避免把陌生文件当原文件还原已拦截（链接与备份均未动）", backup)
		}
	}
