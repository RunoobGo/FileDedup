...
func TestMatchPathStarVsDoubleStar(t *testing.T) {
	cases := []struct {
		...
		// M64 补强：前缀必须按**段边界**成立。缺了 Under 里那条 "/" 时
		// "a/b" 会把邻居 "a/bc" 整棵吞掉（用户数据静默漏扫），而既有用例
		// 全是"深层后代"形态，测不出这一格——变异 M-M64-c 下本包曾一路绿。
		{"字面前缀只认后代不认邻居", "a/b", "a/bc/d", false},
		{"字面前缀命中目录自身", "a/b", "a/b", true},
		{"单层 * 不跨段", "a/*", "a/x", true},
