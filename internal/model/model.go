...
type OpsResult struct {
	...
	// TrashedBytes 移入回收站涉及的字节数（2026-09-21 新增，与另三个口径互斥）。
	//
	// 这是 2026-09-19 硬链接那笔假账的**同构复发**（M22，04 §6.8.8）：
	// trash 的文件并没有离开磁盘——同卷进回收站只是一次改名（数据落到
	// `$Recycle.Bin` / `~/.Trash` / XDG 目录），跨卷也只是把数据搬到另一个卷的
	// 回收站；两种情形下**磁盘总量都一分未减**，要等用户清空回收站才释放。
	// 修正前 trash 落进 Reclaimed，于是结果条写"成功 N（释放 X）"，
	// 而紧挨着的按钮是"打开回收站"——文件就在回收站里，用户按这句话去核对
	// 总量必然对不上（与硬链接当年的自相矛盾一字不差）。
	//
	// 计数口径（别读成"释放"）：数的是**进了回收站的字节数**。措辞见
	// frontend/src/utils/opdisplay.ts 的 case 'trash'。
	TrashedBytes uint64
