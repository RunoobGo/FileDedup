...
#   打印以 "SKIP（跳过，非通过）:" 开头的那一行，CI 按「行首 SKIP + rc=2」双条件认领）；
#   1 = 真失败。跳过**不**算通过，见 skip() 的说明。
#
...
# skip 以退出码 2 + **行首** "SKIP（跳过，非通过）:" 标记自证，绝不与全绿（0）同形。
# 前缀字样必须与这里逐字一致：契约的两侧（ci.yml 的 `grep -q '^SKIP'`、
# smoke-symlink-assert.sh 的 skip_anchored）都按**行首**认领，写在中段不算数（M142）。
#
